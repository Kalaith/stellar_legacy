...
export const ChronicleViewer: React.FC<ChronicleViewerProps> = React.memo(
  ({ onEntrySelect, onHeritageGenerate }) => {
    ...
    const handleHeritageGenerate = useCallback(
      (entry: ChronicleEntry) => {
    ...
    if (error) {
      return (
        <TerminalWindow title="CHRONICLE ACCESS ERROR" className="min-h-96">
          <TerminalText className="warning">ERROR: {error}</TerminalText>
          <button
            ...
            {chronicleViewer.entries.map((entry, index) => (
              <div
                ...
                className={`border border-gray-600 p-4 rounded cursor-pointer hover:border-green-400 transition-colors ${
                  chronicleViewer.selectedEntry?.missionId === entry.missionId
                  ...
                  <div className="mt-2 space-y-1 text-sm">
                    <div>
                      ...
                      <span className="text-green-400">
                        {chronicleViewer.selectedEntry.startingLegacy}
                      </span>
                    </div>
                    <div>
                      Target:{' '}
                      <span className="text-green-400">
                        {chronicleViewer.selectedEntry.targetSystem}
                      </span>
                    </div>
                    <div>
                      Duration:{' '}
                      <span className="text-green-400">
                        {ChronicleUtils.formatDuration(
                          chronicleViewer.selectedEntry.actualDuration
                        )}
                      </span>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-1 text-sm">
                    <div>
                      ...
                      <span
                        className={ChronicleUtils.getSuccessLevelColor(
                          chronicleViewer.selectedEntry.successLevel
                        )}
                      >
                        {ChronicleUtils.getSuccessLevelText(
                          chronicleViewer.selectedEntry.successLevel
                        )}
                      </span>
                    </div>
                    <div>
                      Dominant Legacy:{' '}
                      <span className="text-green-400">
                        {chronicleViewer.selectedEntry.dominantLegacy}
                      </span>
                    </div>
                  </div>
                </div>
