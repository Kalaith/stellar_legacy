...
const Header: React.FC = React.memo(() => {
  ...
  return (
    <header
      className={`${uiConstants.COLORS.BG_PRIMARY} border-b ${uiConstants.COLORS.BORDER} px-6 py-4`}
    >
        ...
        <div className="flex items-center space-x-6">
          <div className="flex items-center space-x-4">
            ...
            <ResourceItem
              icon="🎖️"
              label="Influence"
              value={Number(resources.influence.toFixed(2))}
            />
          </div>
        </div>
      </div>
...
const ResourceItem: React.FC<ResourceItemProps> = ({ icon, label, value }) => (
  <div
    className={`flex items-center space-x-2 ${uiConstants.COLORS.BG_PRIMARY} px-3 py-2 rounded-lg`}
  >
