...
const ComponentShop: React.FC = React.memo(() => {
  ...
  const canAfford = useCallback((cost: ComponentCost) => {
  ...
  const handleCategorySwitch = useCallback((categoryId: ComponentCategoryType) => {
  ...
  return (
    <div className={`${uiConstants.CARDS.BACKGROUND} ${uiConstants.CARDS.BASE} ${uiConstants.SPACING.CARD_PADDING} ${uiConstants.CARDS.BORDER}`}>
      <h3 className={`text-xl font-bold ${uiConstants.COLORS.TEXT_PRIMARY} ${uiConstants.SPACING.SECTION_MARGIN}`}>Available Components</h3>
        ...
        {currentComponents.map(component => (
          <div key={component.name} className={`${uiConstants.COLORS.BG_SECONDARY} rounded p-4`}>
            <div className="flex justify-between items-start mb-2">
              <h4 className={`${uiConstants.COLORS.TEXT_PRIMARY} font-semibold`}>{component.name}</h4>
              <button
