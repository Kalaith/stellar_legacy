...
const StatItem: React.FC<StatItemProps> = ({ label, value }) => (
  <div
    className={`flex justify-between items-center py-2 border-b ${uiConstants.COLORS.BORDER} last:border-b-0`}
  >
    <span className={uiConstants.COLORS.TEXT_SECONDARY}>{label}:</span>
    <span className={`${uiConstants.COLORS.TEXT_PRIMARY} font-semibold`}>
      {value}
    </span>
  </div>
);
